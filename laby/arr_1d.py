...
def paint(laby, sizeX, sizeY, b_size=40, pensize=7, way=None, crosspoint=None, solution=None):
    ...
    screen = t.Screen()
    width, height = ((sizeX - 1) / 2) * b_size + 40 , ((sizeY - 1) / 2) * b_size + 40
    ...
    def p_laby_base(laby, sizeX, sizeY, t=t, b_size=b_size):
        realX, realY = sizeX + 2, sizeY + 2
        for y in range(1, sizeY+1, 2):
            for x in range(1, sizeX+1, 2):
                cur_pos = x + y * realX
                if li[cur_pos + 1] == 1:
                    t.penup()
    ...
    def draw(laby, sizeX, sizeY):
        t.reset()
        t.tracer(0)
        t.hideturtle()
        t.pensize(3)
        p_laby_base(laby, sizeX, sizeY)
        t.update()
...
if __name__ == "__main__":
    ...
    start = clock()
    li = dig_laby(sizeX, sizeY)
    end = clock()
    if sizeX * sizeY <= 77 * 31:
        print_laby(li, sizeX, sizeY)
    print("Berechnungszeit: %.5f" % (end - start))

    if sizeX * sizeY <= 331 * 201:
        paint(li, sizeX, sizeY, b_size=10)
